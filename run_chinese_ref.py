...
def prepare_ref(lines: List[str], ltp_tokenizer: LTP, bert_tokenizer: BertTokenizer):
    ...
    with tqdm(range(0, len(lines), 100), total=len(lines) / 100, desc="ltp_tokenizering") as pbar:
        for i in pbar:
    ...
    bert_res = []
    with tqdm(range(0, len(lines), 100), total=len(lines) / 100, desc="bert tokenizering") as pbar:
        for i in pbar:
...
def main(args):
    ...
    ltp_tokenizer.add_words(words=new_words)
